...
describe Conversation do
  it "initializes" do
    ...
    events = [Hangouts::Event.new("event_id", "sender_id", "timestamp", %w(one two three))]
    Conversation.new("conversation_id", "timestamp", participants, events).events.should eq(events)
  end
  ...
  describe "#includes_participant?" do
    it "knows when the conversation has the participant" do
      ...
      c.has_participant?(Participant.new("gaia_NEW", "id", "spec", "phone")).should eq(false)
    end
  end
end
